      ...
      <article className="space-y-10">
        <header className="space-y-3">
          ...
          <p className="text-muted-foreground">
            ...
            이다. 스펙·ADR·런북·회의록·용어집을 한곳에 두고, 팀에는 변경 알림을 보내고
            에이전트에는 MCP로 <strong className="text-foreground">필요한 조각만</strong> 읽고 쓰게 한다.
          </p>
          <p className="text-muted-foreground">
            목적은 하나다 — 에이전트가 같은 문서를 세션마다 컨텍스트에 싣느라 태우는 토큰을 줄이는 것.
          </p>
        </header>
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            에이전트에게 스펙을 읽히는 가장 흔한 방법은 파일 전체를 컨텍스트에 넣는 것이다. 그런데
            한 번의 작업에 실제로 필요한 건 대개 섹션 하나고, 다음 세션에 같은 문서를 처음부터 다시
            싣는다. 문서가 자랄수록 이 비용은 작업 수에 비례해 늘어난다.
          </p>
          <p className="text-sm text-muted-foreground">
            Cushion은 그 흐름을 <strong className="text-foreground">목차 한 번 + 섹션 하나씩</strong>
            으로 바꾼다. 목차는 세션당 한 번이라 작업이 늘수록 격차가 벌어진다.
          </p>
          <div className="rounded-lg border bg-muted/30 p-4 text-sm">
            <strong>자기 자신의 문서로 잰 값</strong> — 전량 로드 대비 <strong>20% 미만, 5배 이상 절감</strong>.
            <p className="mt-1 text-xs text-muted-foreground">
              측정 대상이 이 프로젝트의 문서라 문서가 자라면 값도 움직인다. 그래서 숫자를 박아 두지
              않고 <code>pnpm acceptance</code>가 그때그때 다시 잰다.
            </p>
          </div>
        </section>
...
에이전트 ─doc_put→   document_versions  (이력)   ─→     웹 화면
          ...
          <p className="text-sm text-muted-foreground">
            <strong className="text-foreground">문서의 원본이 여기 있다.</strong> git 레포의{" "}
            <code>.md</code>를 비추는 미러가 아니다. 남의 레포 문서를 고치려면 클론이 필요했는데,
            읽으려고 클론을 피하는 도구가 쓰려면 클론을 요구하는 셈이라 방향을 뒤집었다.
          </p>
          <p className="text-sm text-muted-foreground">
            그래서 git이 공짜로 주던 것을 대신할 장치가 있다 — 덮어쓰기 전에 이전 본문을 남기는{" "}
            <strong className="text-foreground">이력</strong>, 읽을 때 받은 sha를 쓸 때 되돌려 받아
            어긋나면 거부하는 <strong className="text-foreground">동시 편집 보호</strong>, 그리고
            유일한 백업인 <strong className="text-foreground">내보내기</strong>.{" "}
          ...
          <dl className="space-y-4 text-sm">
            <div>
              ...
              <dd className="text-muted-foreground">
                웹 편집 화면에서도, 에이전트의 <code>doc_put</code>으로도 같은 문서를 만들고 고친다.
                모든 변경이 이력에 남고 되돌릴 수 있다. 삭제도 남는다.
              </dd>
            </div>
            <div>
              <dt className="font-medium">조각만</dt>
              <dd className="text-muted-foreground">
                목차를 먼저 보고 필요한 <code>##</code> 섹션만 가져간다. 안 바뀐 문서를 다시 물으면
                본문 대신 <code>unchanged</code> 다섯 글자로 끝난다.
              </dd>
            </div>
            <div>
              <dt className="font-medium">밀렸을 때만</dt>
              <dd className="text-muted-foreground">
                폴링도 상시 연결도 없다. 문서가 바뀌면 다음 툴 응답 끝에 <code>[stale]</code> 한 줄이
