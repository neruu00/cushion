      ...
      <article className="space-y-10">
        <header className="space-y-3">
          ...
          <p className="text-muted-foreground">
            연결하고 나면 에이전트에게 <code>doc_*</code> 툴이 보여요. 직접 부를 일은 없지만,
            에이전트가 예상과 다르게 굴 때 원인을 찾으려면 동작을 알아야 해요.
          </p>
        </header>
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            서버가 <code>initialize</code> 응답에 실어 보내는 문자열이에요. 이게 툴 사용 순서를
            정해요.
          </p>
...
{`doc_put(library:"cushion", path:"SPEC.md",
        ...
        note:"토큰 회전 규칙 추가")`}
          ...
          <p className="text-sm text-muted-foreground">
            ...
            합치지 않아요. 다시 읽고 다시 쓰면 돼요.
          </p>
          <p className="text-sm text-muted-foreground">
            <code>heading</code>을 주면 그 섹션만 교체해요. 문서 전체를 보내지 않아도 되니 쓰기
            쪽 토큰도 줄어요.
          </p>
        </section>
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            형태예요. LLM을 쓰지 않아요. 요약 모델은 본문만 봐서 “어느 코드가 영향받는지”가
            추측이 되거든요.
          </p>
        </section>
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            문서 화면의 <strong className="text-foreground">이력</strong>에서 버전 사이 변경을
            hunk로 보고, 과거 본문을 전문으로 열고, 되돌릴 수 있어요. 되돌리기는 덮어쓰기가
            아니라 새 저장이라 “언제 무엇으로 되돌렸는지”가 남아요.
          </p>
        </section>
        ...
        <section className="space-y-3">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li>
              ...
              <strong className="text-foreground">요청 시점에 멤버 목록을 조회</strong>해서
              판정하므로, 멤버에서 빼면 재발급 없이 즉시 차단돼요
            </li>
            <li>
            ...
            <li>
              ...
              유출되면 문서 훼손까지 가능하고, 복구 근거는 이력뿐이에요
            </li>
          </ul>
        </section>
