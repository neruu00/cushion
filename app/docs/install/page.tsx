    ...
    <DocsTransition>
      <article className="space-y-10">
        ...
        <section className="space-y-3">
          <h2 className="text-lg font-medium">먼저 토큰을 받는다</h2>
          <p className="text-sm text-muted-foreground">
            <Link href="/settings/tokens" className="underline underline-offset-4">
              /settings/tokens
            </Link>
            에서 발급하면 아래 ①의 명령이 <strong className="text-foreground">토큰이 박힌 채로</strong>{" "}
            나온다. 토큰은 그때 한 번만 보인다 — 저장은 sha256 해시로만 하므로 다시 꺼낼 수 없고,
            잃어버리면 재발급이다(재발급하면 이전 토큰은 즉시 죽는다).
          </p>
        </section>
        ...
        <section className="space-y-3">
          <h2 className="text-lg font-medium">③이 가장 안 보이는데 가장 중요하다</h2>
          <p className="text-sm text-muted-foreground">
            로컬에 문서가 없는 게 이 구조의 정상 상태다. 그래서 에이전트는 아무 표시가 없으면
            “문서가 없네” 하고 그냥 지나간다. <code>AGENTS.md</code>의 한 줄이{" "}
            <strong className="text-foreground">유일한 방아쇠</strong>고,{" "}
            <code>/cushion-use</code>가 그 줄을 넣는다.
          </p>
          <p className="text-sm text-muted-foreground">
            팀원이 이미 <code>AGENTS.md</code>를 커밋해 뒀다면 합류자는 ①만 하면 된다.
          </p>
        </section>
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            개인 연결은 위 3단계로 끝난다. 레포에 커밋해 팀 전원에게 배포하고 싶다면 라이브러리
            화면의 설정에서 <code>.mcp.json</code> 스니펫을 받는다.{" "}
            <strong className="text-foreground">거기에는 토큰을 박지 않는다</strong> — 커밋되는
            파일이라 환경변수 확장(<code>{"${CUSHION_TOKEN}"}</code>)을 쓴다.
          </p>
          <p className="text-sm text-muted-foreground">
            주의할 게 하나 있다. 프로젝트 스코프 설정은 user 스코프를 덮으므로, 개인이 이미 붙여 둔
