      ...
      <article className="space-y-10">
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            Claude Code와 Antigravity, Codex CLI 가운데 실제로 쓰는 것만 고르면 돼요. 토큰은 그때
            한 번만 보여요. 서버에는 sha256 해시만 저장되기 때문에 다시 꺼낼 수 없고, 잃어버리면
            재발급해야 해요. 재발급하면 이전 토큰은 그 즉시 무효가 돼요.
          </p>
        </section>
        ...
        <section className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            헤더만 있으면 되는 평범한 Streamable HTTP 서버라서{" "}
            <strong className="text-foreground">클라이언트를 가리지 않아요.</strong> ①에서 실제로
            쓰는 클라이언트를 고르세요. ②의 스킬도 같은 탭 구성이고, ③은 어떤 클라이언트를 쓰든
            완전히 같아요.
          </p>
          <OnboardingSteps
            skillsUrl={skillsUrl()}
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            로컬에 문서가 없는 것이 이 구조의 정상 상태예요. 그래서 에이전트는 아무 표시가 없으면
            “문서가 없네” 하고 그냥 지나가요. <code>AGENTS.md</code>에 적어 둔 한 줄이{" "}
            <strong className="text-foreground">에이전트가 Cushion을 찾게 만드는 유일한 신호</strong>
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            개인 연결은 위의 세 단계로 끝나요. 레포에 커밋해서 팀 전원에게 배포하고 싶다면
            라이브러리 화면의 설정에서 <code>.mcp.json</code> 스니펫을 받으세요.{" "}
            <strong className="text-foreground">그 파일에는 토큰을 넣지 않아요.</strong> 커밋되는
            파일이라서 환경변수 확장(<code>{"${CUSHION_TOKEN}"}</code>)을 써요.
          </p>
          <p className="text-sm text-muted-foreground">
            한 가지 주의할 점이 있어요. 프로젝트 스코프 설정은 user 스코프를 덮어쓰기 때문에,
            개인이 이미 연결해 둔 상태에서 <code>.mcp.json</code>만 받으면{" "}
            <code>CUSHION_TOKEN</code>이 없는 한 그 프로젝트 안에서만 401 응답을 받게 돼요.
          </p>
        </section>
