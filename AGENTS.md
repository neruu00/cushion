...
AI 스펙주도 개발용 스펙 공유 계층. 각 프로젝트 git 레포의 `.md`를 미러링해서, 팀에는 변경 알림을 보내고 에이전트에는 MCP로 **필요한 조각만** 제공한다. 목적은 하나 — **에이전트가 같은 문서를 반복해서 컨텍스트에 싣느라 태우는 토큰을 줄이는 것.**
...
| | 접두사 | 주체 | 방향 | 스코프 |
|---|---|---|---|---|
| sync token | `cshn_sync_` | 레포(CI) | 쓰기 — 미러 갱신 | 그 레포 하나 |
| access token | `cshn_pat_` | 사람 | 읽기 — 스펙 조회 | 그 사람의 모든 레포 |

섞으면 CI 토큰 유출이 곧 전체 스펙 열람이 된다. 양쪽 다 **sha256 해시만 저장**하고 발급 순간 1회만 노출한다. 재생성 = 새 값 + 기존 `revoked_at` 설정.

### 5. 원본은 git이다. 여기는 미러다

Cushion은 **읽기 전용**이다. 문서 수정은 각 프로젝트 레포에서 PR로 한다.
따라서 문서 쓰기 API·편집 UI·버전 테이블·충돌 해결을 **만들지 않는다.** 만들고 싶어지면 그건 이 설계를 뒤집는 것이고, `SPEC.md` §2를 먼저 읽어야 한다.

`documents`는 캐시이므로 언제든 `workflow_dispatch` 전체 동기화로 재구축 가능해야 한다.

### 6. 삭제 동기화를 빠뜨리지 않는다

스펙 파일이 지워졌는데 미러에 남으면 **에이전트가 삭제된 스펙을 읽는다.** 이 도구가 없애려는 드리프트를 이 도구가 만드는 셈이다. sync 페이로드는 `changed`와 `deleted`를 **둘 다** 실어야 하고, 수신부는 둘 다 처리해야 한다.
...
- **LLM diff 요약** — 요약 모델은 diff만 보므로 "어느 코드가 영향받는지" 판단이 추측이 된다. 구조적 요약(경로 + 헤딩 + 증감)으로 간다 (D-002)
- **문서 CRUD / 편집 UI** — 원본이 git이다 (D-001)
...
| Zod 스키마 | `*.schema.ts` | `sync.schema.ts` |
| 타입 | `*.type.ts` | `spec.type.ts` |

### TypeScript

- `strict: true`. `any` 금지 — 불가피하면 `unknown` + 타입 가드
- 외부 입력(sync 페이로드, MCP 툴 인자, formData)은 `as`로 단언하지 말고 **Zod `safeParse`로 검증**한다. 이게 신뢰 경계다
...
마크다운 렌더는 **서버 컴포넌트에서** 한다(`react-markdown`) — 클라이언트 번들 0. 읽기 전용 페이지에 에디터 런타임을 싣지 않는다.
