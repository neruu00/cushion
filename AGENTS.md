...
**알림은 편집마다 나가지 않는다 — 디바운스된다.** 조용하다 처음 온 편집은 즉시 나가고,
그 뒤 `CUSHION_NOTIFY_WINDOW_MIN`분(기본 5) 안의 연속 편집은 쌓였다가 창이 닫힌 뒤 첫 쓰기가
한 통으로 묶어 보낸다. 에이전트는 섹션 단위로 연달아 쓰기 때문에 안 묶으면 채널이 도배되고,
도배된 알림은 사람이 안 읽어서 없는 것과 같아진다. 그러니 **"편집했는데 알림이 안 왔다"는
버그가 아니다.** 대기분은 `sync_events.notified_at is null`로 남아 있고 다음 발송에 실린다.

크론이 없어서 **버스트의 꼬리는 다음 쓰기까지 지연된다.** 없어지지는 않는다.
크론을 얹기로 하면 `maybeNotify`를 그대로 부르면 된다 — 스키마가 같다.
