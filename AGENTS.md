...
웹으로는 <https://cushion-archive.vercel.app/libraries/cushion>.
...
**알림은 편집마다 한 통 나간다. 묶지 않는다.**

전에는 `CUSHION_NOTIFY_WINDOW_MIN`분 창으로 디바운스했다. 도배는 줄었지만 창이
**라이브러리 단위**여서, 그 안에 들어온 남의 무관한 작업까지 한 통에 섞였다. 묶음의 기준이
"같은 작업"이 아니라 "같은 시간"이라 사람이 읽고 오해했다. 시간은 작업의 대용물로 너무 거칠다.

**대신 두 가지를 알고 감수한다.** 에이전트는 섹션 단위로 연달아 쓰므로 채널이 시끄럽다
(실측 49건 기준 10회 → 49회). 그리고 `notifyChange`가 쓰기 경로에서 `await`되므로
**쓰기마다 웹훅 왕복을 기다린다**(채널당 최대 5초). 조용히 하려면 그 라이브러리의 웹훅 URL을
비운다 — 채널이 없으면 첫 줄에서 바로 빠진다.

**묶기를 되살릴 거라면 시간이 아닌 축으로 한다.** 최소한 `author`를, 가능하면 MCP 세션
단위로. 시간 창만으로 되돌리면 같은 문제로 돌아온다.

`notified_at`은 그대로 찍는다. 단 **한 채널이라도 실제로 받았을 때만** 찍는다 —
전부 실패했으면 null로 남겨야 "안 나갔다"가 사실로 남는다. 재발송 장치는 없다.
