...
export interface Library {
  ...
  /**
   * 로그인 없이 문서를 **읽을** 수 있는가 (D-024). 쓰기는 이 값과 무관하게 멤버만이다.
...
/**
 * **읽기 전용** 조회 결과. 공개 라이브러리는 비로그인도 여기까지는 통과한다 (D-024).
 *
...
/**
 * 문서를 **읽기 위한** 조회. 멤버이거나, 라이브러리가 공개면 통과한다 (D-024).
 *
...
/**
 * 초대 링크 토큰 → 그 라이브러리. 무효화됐거나 형태가 안 맞으면 null (D-023).
