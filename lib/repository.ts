 ...
 * @description 레포 생성의 **유일한 경로**. 웹 폼도 MCP `repo_create`도 여기로 모인다.
 *
 ...
 */
import { createRepositorySchema } from "@/lib/repository.schema";
 ...
 */
export async function createRepositoryFor(
  email: string,
  ...
  if (error || !repo) {
    console.error("createRepositoryFor", error);
  ...
  const { error: memberError } = await supabase
    ...
    .insert({ repository_id: repo.id, email: email.toLowerCase() });

  if (memberError) {
    console.error("createRepositoryFor: 첫 멤버 등록 실패, 보상 삭제", memberError);
