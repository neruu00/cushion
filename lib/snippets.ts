...
jobs:
  sync:
    ...
    # 미러는 기본 브랜치를 비춘다. 작업 브랜치 push까지 받으면 머지도 안 된 문서가
    # 미러를 덮어쓰고, 에이전트는 그걸 확정된 스펙으로 읽는다.
    if: >-
    ...
    steps:
      ...
      - name: 스펙 변경 수집 후 Cushion으로 전송
        ...
        run: |
          ...
          # 붙여넣기 부산물은 사람이 고치게 두지 않는다 — 공백·따옴표·'Bearer '를 여기서 턴다.
          TOKEN="$CUSHION_SYNC_TOKEN"
          ...
          case "$TOKEN" in
            ...
            cshn_pat_*)
              ...
              exit 1 ;;
            *)
              # 값은 찍지 않는다. 길이만으로도 '엉뚱한 걸 붙여넣었는지'는 바로 갈린다 (정상 = 53자)
              echo "::error::sync 토큰 형식이 아니다 (정리 후 길이 \${#TOKEN}자, 정상은 53자). /admin > 재발급으로 받은 cshn_sync_ 값을 그대로 넣을 것"
              exit 1 ;;
          esac
