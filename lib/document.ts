 ...
 * 실패 사유. **문장이 아니라 키다.**
 *
 * 이 함수의 결과를 웹 화면과 MCP가 함께 쓰는데, 웹은 보는 사람의 언어로 말해야 하고
 * MCP는 영어 한 벌이다. 여기서 문장을 만들어 버리면 둘 중 하나는 반드시 틀린 언어가 된다 —
 * 사유만 돌려주고 문장은 경계(서버 액션 / MCP 핸들러)에서 만든다.
 *
 * 값은 `lib/i18n.*.ts`의 `errors` 키와 같다.
 */
...
async function conflictNow(libraryId: string, path: string): Promise<WriteResult> {
  ...
  if (!data) {
    return { ok: false, code: "conflict", reason: "deleted_meanwhile" };
  }
  return {
    ok: false,
    code: "conflict",
...
export async function putDocument(input: {
  ...
  if (existing && input.baseSha !== existing.content_sha) {
    return {
      ...
      reason: input.baseSha ? "changed_meanwhile" : "needs_base_sha",
    };
  }
  ...
  if (input.heading !== undefined) {
    if (!existing) {
      return { ok: false, code: "invalid", reason: "section_needs_document" };
    }
  ...
  if (existing) {
    ...
    if (error) {
      ...
      return { ok: false, code: "failed", reason: "save_failed" };
    }
...
export async function deleteDocument(input: {
  ...
  if (error) {
    ...
    return { ok: false, code: "failed", reason: "delete_failed" };
  }
